...
| 用例编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 |
|:--|---------|---------|---------|---------|
| INT-DEDUCT-003 | 完整扣款配置流程 | 部门存在 | 1. POST /salary-deduct/set 设置迟到扣款=50元<br>2. POST /salary-deduct/set 设置早退扣款=50元<br>3. POST /salary-deduct/set 设置旷工扣款=200元<br>4. POST /salary-deduct/set 设置请假扣款=100元<br>5. POST /leave/set 设置部门假期天数<br>6. GET /salary-deduct 验证4种扣款类型 | 步骤1-5 返回 200<br>步骤6 查询到4条扣款规则 |
| INT-DEDUCT-004 | 扣款规则重复设置 | 同一部门已设置迟到扣款 | 1. POST /salary-deduct/set 再次设置同一 deptId+typeNum | 根据业务逻辑处理（覆盖或报错） |
| INT-DEDUCT-005 | 删除扣款规则 → 薪资不再扣款 | 扣款规则已生效 | 1. DELETE /salary-deduct/{id}<br>2. GET /salary 验证薪资不含该扣款 | 扣款取消 |
...
| 用例编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 |
|:--|---------|---------|---------|---------|
| INT-LEAVE-CFG-001 | 设置部门各假期天数 | 部门存在 | 1. POST /leave/set 技术部设置：个人假5天/病假30天/婚假3天/产假158天<br>2. GET /leave 验证假期配置 | 假期天数正确保存 |
| INT-LEAVE-CFG-002 | 修改假期天数 → 已验证请假不受影响 | 员工已有已批准的请假 | 1. PUT /leave 修改假期天数<br>2. 查询历史请假记录 | 历史请假不变（快照模式） |
...
| 用例编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 |
|:--|---------|---------|---------|---------|
| INT-OVER-CFG-001 | 设置部门加班规则 | 部门存在 | 1. POST /overtime/set 设置工作日加班=1.5倍<br>2. POST /overtime/set 设置休息日加班=2倍<br>3. POST /overtime/set 设置法定假日加班=3倍<br>4. GET /overtime 验证规则 | 3条加班规则正确保存 |
| INT-OVER-CFG-002 | 加班规则修改 → 员工新加班生效 | 旧规则存在 | 1. PUT /overtime 修改工作日倍率 1.5→2.0<br>2. POST /staff-overtime 创建新加班<br>3. 验证加班费按新倍率计算 | 新加班按新规则计算 |
...
| 用例编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 |
|:--|---------|---------|---------|---------|
| INT-HOME-001 | 登录后首页数据加载 | 已有员工/部门/考勤数据 | 1. POST /login 登录<br>2. GET /home 获取仪表盘数据 | 返回员工总数/部门数/月考勤统计等汇总数据 |
| INT-HOME-002 | 无数据时首页展示 | 空数据库 | 1. 登录<br>2. GET /home | 返回空统计而非报错 |
...
| 用例编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 |
|:--|---------|---------|---------|---------|
| INT-AUTH-001 | Token 过期后自动拒绝 | Token 已过期 | 1. 使用过期 Token 调用接口 | 返回 401 或 403 |
| INT-AUTH-002 | 伪造 Token 被拒绝 | — | 1. 使用随机字符串作为 Token | 返回 401 |
| INT-AUTH-003 | 缺少 Token 被拒绝 | — | 1. 不带 Authorization 头调用受保护接口 | 返回 401 |
...
| 用例编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 |
|:--|---------|---------|---------|---------|
| INT-E2E-SAL-001 | 完整薪资计算：基础工资 + 加班费 − 社保 − 迟到扣款 | 全部配置已完成 | 1. 设置员工薪资 10000<br>2. 设置社保(个人缴纳约10.5%)<br>3. 记录迟到1次(扣50)<br>4. 记录加班4小时(加班费400)<br>5. 查询当月薪资 | 实际到手 = 10000 + 400 − 1050 − 50 = 9300 |
| INT-E2E-SAL-002 | 完整薪资：旷工扣款 + 请假扣款 | 全部配置已完成 | 1. 基础工资 10000<br>2. 旷工1天(扣200)<br>3. 请假2天(扣200)<br>4. 查询当月薪资 | 实际到手 = 10000 − 社保 − 200 − 200 |

### S6.2 请假→考勤→薪资全链路

| 用例编号 | 测试场景 | 前置条件 | 操作步骤 | 预期结果 |
|:--|---------|---------|---------|---------|
| INT-E2E-LEAVE-001 | 请假审批通过 → 考勤标记休假 → 薪资扣除 | 全链路配置就绪 | 1. 员工申请3天请假<br>2. 主管审批通过<br>3. 验证考勤表该3天标记为LEAVE<br>4. 查询薪资含假期扣款 | 全链路数据一致 |
...
| 日期 | 版本 | 变更内容 |
|------|------|---------|
| 2026-06-12 | v1.0 | 初始版本，37 个用例 |
| 2026-06-13 | v2.0 | 补充扣款配置、假期配置、加班规则、首页仪表盘、安全认证、跨模块数据流回归，增至 52 个用例 |
